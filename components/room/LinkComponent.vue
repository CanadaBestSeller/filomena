<template>
  <div class="shadow-sm bg-light pt-2 pb-1 d-flex flex-column justify-content-center room-link-div text-center">
    <p style="font-family: 'Cera Round Pro Bold', sans-serif" class="my-0">{{ $t('lobby.inviteFriends') }}</p>
    <b-link ref="roomLink" @click="copyLinkToClipboard()" class="room-link">
      {{ roomLink }}
    </b-link>
    <p class="my-0" v-if="copied" style="font-family: 'Cera Round Pro Bold', sans-serif">{{ $t('general.copied') }}</p>
    <p class="my-0" v-else @click="copyLinkToClipboard()">{{ $t('general.clickToCopyToClipboard') }}</p>
  </div>
</template>

<script>
export default {

  data() {
    return {
      copied: false
    }
  },

  computed: {
    roomLink () { return 'thumbs.up.railway.app/' + this.roomId },
  },

  methods: {
    copyLinkToClipboard () {
      navigator.clipboard.writeText(this.roomLink)

      const roomLinkRef = this.$refs.roomLink
      roomLinkRef.$el.style.transition = 'all 0.3s ease-out'
      roomLinkRef.$el.style.opacity = .3
      this.copied = true
      setTimeout(() => {
        roomLinkRef.$el.style.transition = 'all 0.3s ease-out'
        roomLinkRef.$el.style.opacity = 1
      }, 300)
      setTimeout(() => { this.copied = false }, 1000)
    }
  },

  props: [
    'roomId'
  ]
}
</script>

<style>
.room-link-div {
  position: fixed;
  width: 100vw;
  top: 0;
  z-index: 1;
  left: 50%;
  transform: translateX(-50%);
}

.room-link {
  font-family: "Cera Round Pro Bold", sans-serif;
  font-size: 1.5rem;
}
</style>
