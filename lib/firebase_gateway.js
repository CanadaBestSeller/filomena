...
export async function startGame(docRef) {
  await docRef.update({
    status: 'GAME',
...
export function generateQuestions(questionCount) {
  ...
  for (let i = 0; i < questionCount; i++) {
    result.push(generateRandomQuestionStatic())
  }
  return result
}


export function generateRandomQuestionStatic() {
...
export function generateRandomQuestionDynamic1(name) {
  const dynamicQuestionIds = Object.keys(en.questions.static)
...
export async function transitionAfterSummary(docRef, doc) {
  ...
  if (hasMoreQuestions) {
    await docRef.update({ gameStatus: 'ANSWER', currentQuestionIndex: doc.currentQuestionIndex+1, currentGameStatusTimestampEpochSec: getCurrentTimeEpochSec() })
  } else {
