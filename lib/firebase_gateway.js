...
export async function transitionAfterAnswer(docRef) {
  // TODO everyone player is making this call. Only make this call if gameStatus != 'VOTE'
...
export async function transitionAfterVote(docRef, doc) {
  // TODO everyone player is making this call. Only make this call if gameStatus != 'SUMMARY'
...
export async function transitionAfterSummary(docRef, doc) {
  // TODO everyone player is making this call. Only make this call if gameStatus != 'SUMMARY' or 'FINISHED'
...
export function loveQuestion(questionId) {
  ...
  const questionRatingRef = db.collection("questionRating");

  const incrementValue = firebase.firestore.FieldValue.increment(1);
  questionRatingRef.doc(questionId).set({ loves: incrementValue }, { merge: true });
}
...
export function dislikeQuestion(questionId) {
  ...
  const incrementValue = firebase.firestore.FieldValue.increment(1);
  questionRatingRef.doc(questionId).set({ dislikes: incrementValue }, { merge: true });
}
